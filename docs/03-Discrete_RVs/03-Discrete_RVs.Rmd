...
Let $Y$ be a discrete random variable with probability mass function given by $$p(y)=\begin{cases}1/4& x=0\\ 1/2& x=1\\ 1/8 & x=2\\ 1/8 & x=3\end{cases}$$
...
.pull-right[
Using the definition of expected value, $$E(X)=1.2$$
]
...
Using the same setup as before with the *Eurasian Lynx* example...
...
- 1,000 tickets contain a price of &#36;5 each 

- 100 tickets have a prize of &#36;10 each

- 5 tickets have a price of &#36;1,000 each
...
- tossing "scissors" in a game of rock-paper-scissors
...
Random variables that follow a **geometric distribution** are associated with experiments that are *similar* to the inomial experiment:
...
Let $Y=$ trial on which we *first* observe a five after tossing a fair die

**Simulating E(Y)**:

```{r}
first_five = replicate(10000, {
...
(Try this "by-hand" and via simulation!)
...
A random variable $Y$ is said to have a **hypergeometric distribution** if $$P(Y=y)=p(y)=\frac{\binom{r}{y}\binom{N-r}{n-y}}{\binom{N}{y}},$$ where $y$ is an integer $0,1,2,\dots,n$ such that $y\leq r$ and $n-y\leq N-r$. 
...
(Calculate these by-hand or via simulation!)
...
- $P(Y\leq 0)=P(Y=0)=p(0)=e^{-1}$

- $P(Y\leq 1)=P(Y=0)+P(Y=1)=p(0)+p(1)=e^{-1}+e^{-1}$

- $P(Y\leq 2)=p(0)+p(1)+p(2)=e^{-1}+e^{-1}+\frac{1}{2}e^{-1}$

- $P(Y\leq 3)=p(0)+p(1)+p(2)+p(3)=e^{-1}+e^{-1}+\frac{1}{2}e^{-1}+\frac{1}{6}e^{-1}$
...
- Let's derive this using the definition: $m(t)=E(e^{tY})$
...
- **Hint**: This is equal to $\sum_{y}e^{ty}p^{y}(1-p)^{1-y}$, where $y$ equals 0 or 1. 
