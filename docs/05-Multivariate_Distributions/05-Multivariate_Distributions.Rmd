...
Other types of distributions we'll look at:

- **Marginal distributions**: Individual distribution of $Y_{1}$, ignoring the values of $Y_{2}, Y_{3},\dots$

- **Conditional distributions** of $Y_{1}$, given $Y_{2}=y_{2}$: The updated distribution for $Y_{1}$ after observing $Y_{2}=y_{2}$
...
Let $X$ and $Y$ be **continuous** random variables with joint distribution function $F(x,y)$. If there exists a nonnegative function $f(x,y)$, such that $$F(x,y)=\int^{x}_{-\infty}\int^{y}_{-\infty}f(t_{1},t_{2})\,dt_{2}dt_{1},$$ for all $-\infty<x<\infty$, $-\infty<y<\infty$, then $X$ and $Y$ are **jointly continuous random variables**. 
...
1. What is $P(Y_{1}<1, Y_{2}>5)$?

2. What is $P(Y_{1}+Y_{2}<3)$?
...
1. Find the marginal density functions for $Y_{1}$ and $Y_{2}$. 
...
Let $Y_{1}$ and $Y_{2}$ have the joint probability density function $$f(y_{1},y_{2})=\begin{cases}6y_{1}^{2}y_{2},&0\leq y_{1}\leq y_{2}\ \text{and}\ y_{1}+y_{2}\leq 2\\0,&\text{elsewhere}.\end{cases}$$

1. Find the marginal distribution of $Y_{1}$, $f_{1}(y_{1})$. What *named* probability distribution does this follow?

2. Find $f(y_{2}\mid y_{1})$ and use this to find $P(Y_{2}\leq 1\mid Y_{1}=1/2)$. 
...
Let $X$ have distribution function $F_{X}(x)$, $Y$ have distribution function $F_{Y}(y)$, and $X$ and $Y$ have *joint* distribution function $F(x,y)$. Then $X$ and $Y$ are said to be **independent** if and only if $$F(x,y)=F_{X}(x)F_{Y}(y).$$ If $X$ and $Y$ are not independent, they are said to be **dependent**. 
...
- $Corr(X_{1},X_{2})=0.75\implies Cov(X_{1},X_{2})=0.75\sqrt{1}\sqrt{4}=1.5$
]
