...
- $P(Y\leq 0)=P(Y=0)=p(0)=e^{-1}=\boxed{0.37}$

- $P(Y\leq 1)=P(Y=0)+P(Y=1)=p(0)+p(1)=e^{-1}+e^{-1}=\boxed{0.74}$

- $P(Y\leq 2)=p(0)+p(1)+p(2)=e^{-1}+e^{-1}+\frac{1}{2}e^{-1}=\boxed{0.92}$

- $P(Y\leq 3)=p(0)+p(1)+p(2)+p(3)=e^{-1}+e^{-1}+\frac{1}{2}e^{-1}+\frac{1}{6}e^{-1}=\boxed{0.98}$
...
For $Y\sim \text{Poisson}(1)$, $$F(y)=P(Y\leq y)=\begin{cases}0&y<0\\0.37&0\leq y<1\\0.74& 1\leq y<2\\0.92& 2\leq y<3\\0.98&3\leq y<4\\ \dots&\dots\end{cases}$$
...
Instead of modeling the **number** of calls received in an hour, what if we modeled the **lengths** of calls?
...
Is $Y$ has density function $$f(y)=\begin{cases}(3/2)y^2+y,&0\leq y\leq 1,\\0,&\text{elsewhere}\end{cases}$$find the **mean** and **variance** of $Y$. 
...
The $\text{Normal}(\mu, \sigma^{2})$ distribution has PDF $$f(y)=\frac{1}{\sqrt{2\pi\sigma^{2}}}e^{-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^{2}}$$
...
- The **standard normal CDF**, with $\mu=0$ and $\sigma=1$ is so widely-used that it gets its own notation: $\Phi(\cdot)$!
...
**Theorem** (68-95-99.7 Rule): If $Y\sim \text{Normal}(\mu,\sigma^{2})$, then:

- $P(|Y-\mu|<\sigma)\approx 0.68$

- $P(|Y-\mu|<2\sigma)\approx 0.95$

- $P(|Y-\mu|<3\sigma)\approx 0.997$
...
Sometimes we will want to model random variables that are:

- **right-skewed** (non-symmetric)
...
- amount of rainfall accumulated in a given area per day

- the time it takes a mechanic to service your car

- the time Moose needs to wait in between receiving treats

<br>

These can all be adequately modeled by the **gamma distribution**!
...
- The **hazard function**, $h(t)$, provides a *conditional density* of an event, given that the event *has not yet occurred* prior to time $t$: $$h(t)=\lim_{x\to\infty}\frac{1}{x}\frac{P(t\leq T< t+x)}{P(T\geq t)}=-\frac{d}{dt}\log S(t)=\frac{f(t)}{S(t)}$$
...
- **Hint**: The hazard function is **constant** for the exponential distribution. 
...
```{r}
Y = rbeta(n = 1000, shape1 = 2, shape2 = 8)
mean(Y)
var(Y)
```
